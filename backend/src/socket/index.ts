...
import {
  ...
  reportContentSchema,
  submitAnswerSchema,
  submitCustomPromptSchema,
  unlockAdultModeSchema,
  ...
  io.on("connection", (socket: Socket) => {
    socket.on("room:create", (payload, ack) => {
      ...
      const engine = createRoom(
        host,
        { vibes, intensity, rounds, timer, ...toggles },
        deviceMode,
      );
    ...
    socket.on("room:join", (payload, ack) => {
      ...
      const nameTaken = Object.values(engine.state.players).some(
        (p) =>
          p.connected &&
    ...
    socket.on("room:start", () => {
      withRoom(socket, (engine) => {
        if (engine.state.hostId !== socket.data.playerId) return;
        engine.start();
        io.to(engine.state.code).emit("room:state", engine.state);
      });
    });
    ...
    socket.on("round:submitAnswer", (payload, ack) => {
      ...
      withRoom(socket, (engine) => {
        const answerPlayerId =
          engine.state.deviceMode === "pass_and_play"
        ...
        const eligible = new Set(
          engine.state.currentPrompt?.gameType &&
            [
              "never_have_i_ever",
              "most_likely_to",
              "vote",
        ...
        const allAnswered = [...eligible].every(
          (id) => id in engine.state.votes,
        );
...
function withRoom(
  socket: Socket,
  fn: (engine: NonNullable<ReturnType<typeof getRoom>>) => void,
) {
