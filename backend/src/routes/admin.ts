...
router.get("/prompts", (req, res) => {
  ...
  if (gameType) results = results.filter((p) => p.gameType === gameType);
  if (ageRating) results = results.filter((p) => p.ageRating === ageRating);
  if (active !== undefined)
...
router.get("/stats", (_req, res) => {
  ...
  res.json({
    ...
    byGameType: ALL_PROMPTS.reduce<Record<string, number>>((acc, p) => {
