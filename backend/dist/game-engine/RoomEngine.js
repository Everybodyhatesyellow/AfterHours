...
export class RoomEngine {
    ...
    static create(code, host, settings, deviceMode = "shared") {
        const state = {
            code,
            deviceMode,
            hostId: host.id,
    ...
    start() {
        ...
        this.state.round = 0;
        const hasEnoughPlayersForVoting = Object.keys(this.state.players).length >= 3;
        const availableModes = hasEnoughPlayersForVoting
            ? CATEGORY_ROTATION
            : CATEGORY_ROTATION.filter((mode) => mode !== "most_likely_to" && mode !== "vote");
    ...
    advanceRound() {
        ...
        if (!mode)
            throw new Error(`No implementation registered for game type "${gameType}"`);
        ...
        if (prompt)
            this.state.usedPromptIds.push(prompt.id);
...
//# sourceMappingURL=RoomEngine.js.map